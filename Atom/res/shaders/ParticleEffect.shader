...
vec2 calculate_gravity(uint gid, float attractor_mass, vec2 attractor_location) {
	...
	float m = (attractor_mass * particles[gid].duration) / d;
...
void main() {
	...
	// Attractor #1
	vec2 g1 = calculate_gravity(gid, 0.0001, p + (0.8 * a));
	particles[gid].velocity += g1 * particles[gid].duration;	// use time

	// Attractor #2
	vec2 g2 = calculate_gravity(gid, 0.0001, p + (0.3 * a));
	particles[gid].velocity += g2 * particles[gid].duration;
